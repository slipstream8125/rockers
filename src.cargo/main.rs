...
fn print_dnf() {
    println!("\n{BOLD}{ITALIC} >>>{CYAN} Fedora repos   {RESET}\n");
}
...
// adjust_idx {{{
fn adjust_idx(a: i32, b: i32, c: i32, d: i32, e: i32) {
    if a == -1 && b == -1 && c == -1 && d == -1 && e == -1 {
        ...
        // only pacman, paru, flatpak
        print!("{ITALIC}\nSelect package [1-{}]: {RESET}", c);
        let _ = io::stdout().flush();
    } else if e == -1 {
        // only pacman, paru, flatpak and apt
        print!("{ITALIC}\nSelect package [1-{}]: {RESET}", d);
        let _ = io::stdout().flush();
    } else {
        // all 5 PMs
        print!("{ITALIC}\nSelect package [1-{}]: {RESET}", e);
        let _ = io::stdout().flush();
    }
}
// }}}
...
fn inst_info_pkg(pm: &Pkgmgrs, pkg: &str) {
    ...
    adjust_idx(x.pos[0], x.pos[1], x.pos[2], x.pos[3], x.pos[4]);

    io::stdin()
        .read_line(&mut input_pkg_str)
        .expect("Enter a valid integer.");
    let input_pkg_num: i32 = input_pkg_str
        .trim()
        .parse()
    ...
    if 1 <= input_pkg_num && input_pkg_num <= x.pos[0] {
        ...
        info_pkgmgr = "flatpak";
    } else if x.pos[2] < input_pkg_num && input_pkg_num <= x.pos[3] {
        info_pkgmgr = "nala";
    } else if x.pos[3] < input_pkg_num && input_pkg_num <= x.pos[4] {
        info_pkgmgr = "dnf";
    } else if input_pkg_num > x.pos[4] {
    ...
    if input_pkg_num > 0 {
        ...
        println!();
        println!(
            "{ITALIC}Info for package {RESET}{HIGHLIGHT}{}{RESET}.",
            info_pkgname
        );
    } else {
        println!("{RED}ERROR: {RESET}{UNDERLINE}Enter a valid number.{RESET}");
...
fn info_pkg(pm: &Pkgmgrs, pkg: &str) {
    ...
    if input_pkg_num > 0 {
        ...
        println!();
        println!(
            "{ITALIC}Fetching info for package {RESET}{HIGHLIGHT}{}{RESET}.",
            info_pkgname
        );
    } else {
        println!("{RED}ERROR: {RESET}{UNDERLINE}Enter a valid number.{RESET}");
...
fn update_pkg(pm: &Pkgmgrs) {
    ...
    for i in 0..pm.name.len() {
        ...
        if pm.name[i] == "pacman" || pm.name[i] == "nala" || pm.name[i] == "dnf" {
            // run with sudo.
            if pm.name[i] == "pacman" {
                print_pacman();
            }
            // TODO
            else if pm.name[i] == "dnf" {
                print_dnf();
            } else {
                print_apt();
            }
        ...
        if pm.name[i] == "nala" {
            ...
            output = Command::new("sh")
                .args([
                    ...
                    &format!(
                        "sudo {} {} --assume-yes",
                        &pm.name[i], &pm.cleanup_cmd[&pm.name[i]]
                    ),
                ])
                .stdout(Stdio::piped())
                .spawn()
            ...
            let mut output1 = Command::new(&pm.name[i])
                .arg("-Qtdq")
                .stdout(Stdio::piped())
                .spawn()
                .expect("");
            ...
            _ = Command::new("sh")
                .args(["-c", &format!("sudo rm -f /var/lib/pacman/db.lck")])
                .spawn();
            if unused_pkgs_str != "" {
                output = Command::new("sh")
                    .args([
                        ...
                        &format!(
                            "sudo {} {} {} --noconfirm",
                            &pm.name[i], &pm.cleanup_cmd[&pm.name[i]], unused_pkgs_str
                        ),
                    ])
                    .stdout(Stdio::piped())
                    .spawn()
            ...
            output = Command::new("sh")
                .args([
                    ...
                    &format!(
                        "{} {} {}",
                        &pm.name[i], &pm.cleanup_cmd[&pm.name[i]], "--assumeyes"
                    ),
                ])
                .stdout(Stdio::piped())
                .spawn()
            ...
            output = Command::new("sh")
                .args([
                    ...
                    &format!(
                        "{} {} {} {}",
                        &pm.name[i], &pm.cleanup_cmd[&pm.name[i]], "--unused", "--assumeyes"
                    ),
                ])
                .stdout(Stdio::piped())
                .spawn()
        ...
        for line in reader.lines() {
            if let Ok(line) = line {
                if !(line.contains("Nothing unused to uninstall")
                    ...
                    || line.contains("Nothing for Nala to remove"))
                {
                    println!("{line}");
                }
            }
...
fn install_pkg(pm: &Pkgmgrs, pkg: &str) {
    ...
    if input_pkg_num > 0 {
        ...
        println!();
        println!(
            "{ITALIC}Installing package {RESET}{HIGHLIGHT}{}{RESET}.",
            inst_pkgname
        );
    } else {
        println!("{RED}ERROR: {RESET}{UNDERLINE}Enter a valid number.{RESET}");
    ...
    if inst_pkgmgr == "pacman" {
        output = Command::new("sudo")
            .arg(&inst_pkgmgr)
            .arg(&pm.install_cmd[inst_pkgmgr])
            .arg(inst_pkgname)
            .arg(noc)
            .arg("--color=always")
            .stdout(Stdio::piped())
        ...
        output = Command::new(&inst_pkgmgr)
            .arg(&pm.install_cmd[inst_pkgmgr])
            .arg(inst_pkgname)
            .arg(noc)
            .arg("--color=always")
...
fn remove_pkg(pm: &Pkgmgrs, pkg: &str) {
    ...
    if input_pkg_num > 0 {
        ...
        println!();
        println!(
            "{ITALIC}Removing package {RESET}{HIGHLIGHT}{}{RESET}.",
            rm_pkgname
        );
    } else {
        println!("{RED}ERROR: {RESET}{UNDERLINE}Enter a valid number.{RESET}");
    ...
    if rm_pkgmgr == "pacman" {
        ...
        output = Command::new("sh")
            .args([
                ...
                &format!(
                    "sudo {} {} {}",
                    &rm_pkgmgr, &pm.remove_cmd[rm_pkgmgr], rm_pkgname
                ),
            ]) // ask for user confirmation for removal.
            .stdout(Stdio::piped())
            .spawn()
            .expect("No such pkg");
        ...
        output = Command::new("sh")
            .args([
                ...
                &format!(
                    "sudo {} {} {} {}",
                    &rm_pkgmgr, &pm.remove_cmd[rm_pkgmgr], rm_pkgname, "--assume-yes"
                ),
            ])
            .stdout(Stdio::piped())
            .spawn()
            .expect("No such pkg");
    } else if rm_pkgmgr == "dnf" {
        output = Command::new("sh")
            .args([
                "-c",
                &format!(
                    "sudo {} {} {} {}",
                    &rm_pkgmgr, &pm.remove_cmd[rm_pkgmgr], rm_pkgname, "-y"
                ),
            ])
            .stdout(Stdio::piped())
            .spawn()
...
fn display_local_pkg(pm: &Pkgmgrs, pkg: &str) -> PkgResult {
    ...
    let mut index = 1;
    let (mut pacman_idx, mut paru_idx, mut flatpak_idx, mut nala_idx, mut dnf_idx) =
        (-1, -1, -1, -1, -1);
        ...
        if let Some(stdout) = output.stdout.take() {
            ...
            for line in reader.lines() {
                ...
                if pm.name[i] == "pacman" && !line.contains("    ") {
                    ...
                    println!(
                        "[{BLUE}{}{RESET}]: {BOLD}{ITALIC}{}{RESET} [{BLUE}{}{RESET}]{RESET}",
                        index,
                        &line[..fwi].replace("[installed]", ""),
                        "pacman"
                    );
                    res_string += &line[..fwi];
                    res_string += "\n";
                    ...
                    println!(
                        "[{GREEN}{}{RESET}]: {BOLD}{ITALIC}{}{RESET} [{GREEN}{}{RESET}]{RESET}",
                        index, line, &pm.name[i]
                    );
                    let fwi = line.find(char::is_whitespace).unwrap_or(line.len());
                    res_string += &line[..fwi];
                    res_string += "\n";
                    ...
                    for i in 0..nala_vec.len() {
                        if nala_vec[i].contains("INSTALLED") {
                            println!(
                                "[{YELLOW}{}{RESET}]: {BOLD}{ITALIC}{}{RESET} [{YELLOW}{}{RESET}]",
                                index + i,
                                &nala_vec[i].replace(" INSTALLED", ""),
                                "nala"
                            );
    ...
    PkgResult {
        res: res_string,
        pos: vec![pacman_idx, paru_idx, flatpak_idx, nala_idx, dnf_idx],
    }
}
// }}}
        ...
        if let Some(stdout) = output.stdout.take() {
            ...
            for line in reader.lines() {
                ...
                if pm.name[i] == "pacman" {
                    ...
                    let fwi = line.find(char::is_whitespace).unwrap_or(line.len());
                    println!(
                        "[{HIGHLIGHT}{}{RESET}]: {BOLD}{ITALIC}{}{RESET} [{BLUE}{}{RESET}]{RESET}",
                        index,
                        &line[..fwi],
                        "dnf"
                    );
            ...
            for i in 0..nala_vec.len() {
                if nala_vec[i].contains("INSTALLED") {
                    println!(
                        "[{HIGHLIGHT}{}{RESET}]: {BOLD}{ITALIC}{}{RESET} [{YELLOW}{}{RESET}]",
                        index + i,
                        &nala_vec[i].replace(" INSTALLED", ""),
                        "nala"
                    );
...
fn main() {
    ...
    if pkgmgr_found("/usr/bin/flatpak") {
        ...
        pm.install_cmd
            .insert(pm.name[2].clone(), "install".to_string());
        pm.search_cmd
            .insert(pm.name[2].clone(), "search".to_string());
        pm.search_local_cmd
            .insert(pm.name[2].clone(), "list".to_string());
        pm.info_cmd
            .insert(pm.name[2].clone(), "remote-info".to_string());
        pm.inst_info_cmd
            .insert(pm.name[2].clone(), "info".to_string());
        pm.update_cmd
            .insert(pm.name[2].clone(), "update".to_string());
        pm.remove_cmd
            .insert(pm.name[2].clone(), "uninstall".to_string());
        pm.cleanup_cmd
            .insert(pm.name[2].clone(), "uninstall".to_string());
    } else {
    ...
    if pkgmgr_found("/bedrock/cross/bin/nala") {
        ...
        pm.install_cmd
            .insert(pm.name[3].clone(), "install".to_string());
        pm.search_cmd
            .insert(pm.name[3].clone(), "search".to_string());
        pm.search_local_cmd
            .insert(pm.name[3].clone(), "list".to_string());
        pm.info_cmd.insert(pm.name[3].clone(), "show".to_string());
        pm.inst_info_cmd
            .insert(pm.name[3].clone(), "show".to_string());
        pm.update_cmd
            .insert(pm.name[3].clone(), "upgrade".to_string());
        pm.remove_cmd
            .insert(pm.name[3].clone(), "uninstall".to_string());
        pm.cleanup_cmd
            .insert(pm.name[3].clone(), "autopurge".to_string());
    } else {
        println!("{BOLD}{ITALIC} 󰄱 {YELLOW} Apt   {RESET}");
    }

    if pkgmgr_found("/bedrock/cross/bin/dnf") {
        println!("{BOLD}{ITALIC} 󰱒 {BLUE} DNF  {RESET}");
        pm.name.push("dnf".to_string());
        pm.install_cmd
            .insert(pm.name[4].clone(), "install".to_string());
        pm.search_cmd
            .insert(pm.name[4].clone(), "repoquery".to_string());
        pm.search_local_cmd
            .insert(pm.name[4].clone(), "search".to_string());
        pm.info_cmd.insert(pm.name[4].clone(), "search".to_string());
        pm.inst_info_cmd
            .insert(pm.name[4].clone(), "info".to_string());
        pm.update_cmd
            .insert(pm.name[4].clone(), "upgrade".to_string());
        pm.remove_cmd
            .insert(pm.name[4].clone(), "remove".to_string());
        pm.cleanup_cmd
            .insert(pm.name[4].clone(), "clean".to_string());
    } else {
        println!("{BOLD}{ITALIC} 󰄱 {BLUE} DNF 󰣛 {RESET}");
    }
    match rockcmd {
        ...
        "-h" | "--help" => banner(),
        _ => print!(
            "{BOLD}Invalid Usage.{RESET} Consult {ITALIC}rock --help{RESET} for more information."
        ),
    }
}
// }}}
